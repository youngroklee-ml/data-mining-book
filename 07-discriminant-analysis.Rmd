...
위 Table \@ref(tab:lda-posterior-result)의 객체 `r paste(lda_posterior_result_df %>% filter(class != predicted_class) %>% select(id), collapse = ", ")`와 같이 선형분류함수가 모든 객체의 범주를 정확하게 추정하지 못하고 오분류가 발생하는 경우가 있다. 이 때 다음과 같이 두 종류의 오분류 비용이 있다고 가정하자.
