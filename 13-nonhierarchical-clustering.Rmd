...
```{r echo=FALSE, warning=FALSE, message=FALSE}
required_packages <- c("tidyverse", "stats", "cluster", "flexclust", "mclust")
map_dfr(
  required_packages, 
  function(x) {
    tibble(
...
estimate_mixture_cov <- function(modelName, W, z, p) {
  K <- ncol(z)
  ...
  if (modelName == "VII") {
    lambda <- map2(W, z, ~ sum(diag(.x %*% solve(M))) / (sum(.y) * p))
    ...
    while (TRUE) {
      i <- i + 1L
      lambda <- map2(W, z, ~ sum(diag(.x %*% solve(M))) / (sum(.y) * p))
      new_obj <- pmap_dbl(list(W = W, z = z, lambda = lambda), 
                          function(W, z, lambda, M, p) {
                            sum(diag(W %*% solve(M))) / lambda +
                              p * sum(z) * log(lambda)
                          },
                          M = M, p = p) %>%
        sum()
...
estimate_mixture_cov("VII", W, z_hat, p)
```

위 `estimate_mixture_cov` 함수에서 "VII"보다 일반화된 "VEI"와 "VEE" 분산-공분산 모형에 대한 추정도 구현되어 있다.

```{r}
# estimate_mixture_cov("VEI", W, z_hat, p)
# estimate_mixture_cov("VEE", W, z_hat, p)
```
...
GMM_Mstep <- function(df, z, modelName = "VII") {
  ...
  var_cov <- estimate_mixture_cov(modelName, W, z, p)
...
위 일련의 과정들을 포함하는 하나의 함수 `GMM_EM`을 아래와 같이 구현해보자.
...
R 패키지 `mclust`를 통해, 위에서 살펴본 VII, VEI, VEE 외에 보다 다양한 분산-공분산 모형을 가정한 군집분석을 수행할 수 있다 [@scrucca2016mclust].
